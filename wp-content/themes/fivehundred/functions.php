	...
	if (class_exists('Deck')) {
		...
		if (isset($the_deck) && $the_deck->project_type !== 'pwyw') {
			...
			if (function_exists('idc_checkout_image')) {
				...
				$action = get_permalink($post_id).'?purchaseform=500&prodid='.$project_id;
...
function fh_color_styles() {
	...
	if ($customized) {
		...
		.ign-supportnow a, .memberdeck button, .memberdeck input[type="submit"], .memberdeck form .form-row input[type="submit"], .memberdeck .button {background-color: '.$primary_color.'; background-color: '.$primary_color.' ;}';
