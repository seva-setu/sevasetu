	...
	<div class="id-product-infobox">
		<div class="product-wrapper">
			...
			<div class="pledge">
				...
				<?php  if (!$custom || ($custom && isset($attrs['project_bar']))) { ?>
					<div class="progress-wrapper">
						<div class="progress-percentage"><?php echo number_format(apply_filters('id_percentage_raised', $mini_deck->rating_per, $the_deck->p_current_sale, $mini_deck->post_id, $mini_deck->project->goal)); ?>% </div>					
						<!-- end progress-percentage -->
						<div class="progress-bar" style="width: <?php echo apply_filters('id_percentage_raised', $mini_deck->rating_per, $the_deck->p_current_sale, $mini_deck->post_id, $mini_deck->project->goal); ?>%"> 
							<!----> 
						</div>
						<!-- end progress bar --> 
					</div>
			...
			<?php  if (!$custom || ($custom && isset($attrs['project_pledged']))) { ?>
				<div class="id-progress-raised"> <?php echo apply_filters('id_price_selection', getTotalProductFund($project_id), $mini_deck->post_id); ?><?php //echo $mini_deck->cCode; ?><?php //echo number_format(apply_filters('id_funds_raised', getTotalProductFund($project_id), $mini_deck->post_id)); ?> </div>
			<?php } ?>
			<?php  if (!$custom || ($custom && isset($attrs['project_goal']))) { ?>
				<div class="id-product-funding"><?php echo $tr_Pledged_Of.' '.apply_filters('id_project_goal', $mini_deck->project->goal, $mini_deck->post_id)/*.$mini_deck->cCode.number_format($mini_deck->project->goal, 0, '.', ',')*/.' '.$tr_Goal ;?> </div>
			<?php } ?>
