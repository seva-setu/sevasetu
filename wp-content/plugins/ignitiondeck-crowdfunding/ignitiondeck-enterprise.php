...
add_filter('ide_company_url', 'ide_company_url');

function ide_company_url($url) {
	if (!stripos($url, 'http://')) {
		$url = 'http://'.$url;
	}
	return $url;
...
function ide_backer_profile() {
	if (isset($_GET['backer_profile'])) {
		...
		if (isset($profile) && $profile > 0) {
			...
			add_filter('the_title', 'ide_backer_profile_title');
...
function ide_backer_profile_display($content) {
	...
	$content .= '<p>'.wpautop($user->description).'</p></div></div>';
				...
				if (!empty($id_orders)) {
					...
					foreach ($id_orders as $id_order) {
						...
						if (!empty($the_project) && !in_array($id_order->product_id, $listed)) {
							...
							$order_content .= '<strong class="backer_project_title"><a href="'.$url.'">'.stripslashes(strip_tags(html_entity_decode(get_the_title($post_id)))).'</a></strong>';
	...
	$content .= (isset($order_count) && $order_count > 0 ? '<div class="backer_data"><p class="backer_supported"><span class="order_count">'.$order_count.'</span> '.__('Projects Supported', 'ignitiondeck').'</p>' : '<div class="backer_data">');
...
function ide_backer_profile_title($title, $sep = null) {
	global $post;
	if (isset($post) && $post->post_title == $title) {
