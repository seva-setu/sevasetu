		...
		foreach ($this->fields as $field) {
			...
			switch($type) {
				...
				case 'wpeditor':
					ob_start();
					if (!empty($label)) {
						echo '<p><label for="'.$id.'">'.apply_filters('fes_'.$name.'_label', $label).'</label>';
					}
					wp_editor(html_entity_decode($value), $id, array('editor_class' => $class, 'textarea_name' => $name, 'media_buttons' => 0, 'textarea_rows' => 6));
					if (!empty($label)) {
						echo '</p>';
					}
					$output .= ob_get_contents();
